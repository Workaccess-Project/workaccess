(() => {
  ...
  function friendlyError(e) {
    ...
    // Billing gate (většinou řeší api.js redirect, ale pro allowlisted stránky chceme text)
    if (status === 402 || code === "TrialExpired" || raw.includes("TrialExpired")) {
      return "Trial vypršel. Některé funkce mohou být uzamčené. Otevři Billing a aktivuj tarif.";
    }

    // RBAC / Auth
    if (status === 403) return "Nemáš oprávnění pro tuto akci (403).";
    if (status === 401) return "Nejsi přihlášen (401). Zkus se znovu přihlásit.";
